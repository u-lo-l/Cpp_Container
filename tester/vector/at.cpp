#include "../container_tester.hpp"
#include <vector>
#include "../../include/ft_vector.hpp"

int main(void)
{
	value_creator<TYPE> val_creator;

	TESTING_NAMESPACE::vector<TYPE> vct(7);

	for (unsigned long int i = 0; i < vct.size(); ++i)
	{
		vct.at(i) = val_creator((vct.size() - i) * 3);
		std::cout << "vct.at(): " << vct.at(i) << std::endl;
		std::cout << "vct[]: " << vct[i] << std::endl;
	}
	printContainer("non const vec", vct);

	TESTING_NAMESPACE::vector<TYPE> const vct_c(vct);

	std::cout << "front(): " << vct.front() << " " << vct_c.front() << std::endl;
	std::cout << "back(): " << vct.back() << " " <<  vct_c.back() << std::endl;

	try {
		vct.at(10) = val_creator(42);
	}
	catch (std::out_of_range &e) {
		std::cout << "Catch out_of_range exception!" << std::endl;
	}
	catch (std::exception &e) {
		std::cout << "Catch exception: " << e.what() << std::endl;
	}

	for (unsigned long int i = 0; i < vct_c.size(); ++i)
	{
		std::cout << "vct_c.at() : " << vct_c.at(i) << std::endl;
		std::cout << "vct_c[]    : " << vct_c[i] << std::endl;
	}
	try {
		std::cout << vct_c.at(10) << std::endl;
	}
	catch (std::out_of_range &e) {
		std::cout << "Catch out_of_range exception!" << std::endl;
	}
	catch (std::exception &e) {
		std::cout << "Catch exception: " << e.what() << std::endl;
	}
	printContainer("const", vct_c);

	return (0);
}
